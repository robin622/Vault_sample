...
public class VaultFileUploadServlet extends HttpServlet {
                ...
                if ("list".equals(operation)) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (itemType.contains("vnd.oasis.opendocument")
                                ...
                                || (itemType.contains("octet-stream") && !itemfileName.contains(".jsp") && !itemfileName
                                        .contains(".inc")) || itemType.contains("image") || itemType.contains("x-gzip")
                                || itemType.contains("x-bzip") || itemType.contains("x-rar") || itemType.contains("zip")
                                || itemType.contains("x-compress") || itemType.contains("x-tar")
                            ...
                            if (itemfileName.lastIndexOf(".") >= 0) {
                                ...
                                if (truefilename.matches("^[0-9a-zA-Z/_/./-]*$")) {
                                    isAllowedType = true;
                                } else {
                                    NameError = true;
                                }
    ...
    private String createPath(HttpServletRequest req) {
        return getServletContext().getRealPath(Attachment.FILEPATH) + "/" + req.getRequestedSessionId();
    }
