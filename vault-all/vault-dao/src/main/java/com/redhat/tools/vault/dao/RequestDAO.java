...
public class RequestDAO {
	...
	public List<Request> get(Request condition) {
		...
		try {
			...
			if (condition.getRequestid() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_REQUESTID,
						new Long(condition.getRequestid())));
			}
			if (condition.getRequestname() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_REQUESTNAME,
						condition.getRequestname()));
			}
			if (condition.getValue() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_VALUE,
						condition.getValue()));
			}
			if (condition.getStatus() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_STATUS,
						condition.getStatus()));
			}
			if (condition.getCreatedby() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_CREATEDBY,
						condition.getCreatedby()));
			}
			if (condition.getOwner() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_OWNER,
						condition.getOwner()));
			}
			if (condition.getVersionid() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_VERSIONID,
						condition.getVersionid()));
			}
			if (condition.getIs_public() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_IS_PUBLIC,
						condition.getIs_public()));
			}
			if (condition.getForward() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_CC_LIST,
						condition.getForward()));
			}
			if (condition.getFrom() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_FROM,
						condition.getFrom()));
			}
			if (condition.getRequestVersion() != null) {
				criteria.add(Expression.eq(Request.PROPERTY_REQUEST_VERSION,
	...
	public List<Request> getPeriodRequest(String username, Date start, Date end) {
		...
		try {
			...
			if(start !=null){
				criteria.add(Expression.gt(Request.PROPERTY_CREATEDTIME,
						start));
			}
			if(end != null){
				criteria.add(Expression.lt(Request.PROPERTY_CREATEDTIME,
						end));
			}
		...
		try {
			...
			if (username != null && username.trim().length()!=0) {
				...
				if(start!=null || end!=null){
					...
					queryStringBuffer.append(" and a.requestid in(select b.requestid from RequestHistory b where b.editedby=? and (b.status = ? or b.status = ? or b.status = ?) and b.isHistory = 0 )");
				}else{
					queryStringBuffer.append("a.requestid in(select b.requestid from RequestHistory b where b.editedby=? and (b.status = ? or b.status = ? or b.status = ?) and b.isHistory = 0 )");
				}
...
//				String queryString =  (a.signedtime between ? and ?) or (a.editedtime between ? and ?)";
	...
	public Request update(Request request) {
		...
		try {
			...
			if (request.getForward() != null) {
				String forward = StringUtil.deFormartCCList(request
						.getForward());
	...
	public List<Request> getAllByStatus(String userEmail, String userName,
			...
			for (Request request : allRequests) {
				...
				if (operation != null && !"".equals(operation)) {
					if ("canView".equals(operation)) {
						...
						if (itemStatus != null
								&& !Request.INACTIVE.equals(itemStatus)) {
							if (owner != null) {
	...
	public List<Request> getAllByStatus(List<Request> allRequests,
			String userEmail, String userName, String operation)
			throws Exception {
			...
			for (Request request : allRequests) {
				...
				if (operation != null && !"".equals(operation)) {
					if ("canView".equals(operation)) {
						...
						else if (isInForward(userEmail, request)
								|| isInSignOff(userEmail, request)) {
							list.add(request);
						}
					}
					else if ("waitingSignOff".equals(operation)) {
	...
	public List<Request> searchByFlowName(String requestName, String userName,
			String userEmail) {
		log.debug("finding all Log instances");
		Session sess = null;
		try {
	...
	public List<Request> advanceSearch(String requestName, String creator,
			String versionid, String productid, String status,
	...
	public List<Request> findByIds(Long[] requestIds) {
		...
		try {
			String queryString = "from Request as a where a.requestid in "
					+ inString + " order by a.requestid desc";
			Query queryObject;
			queryObject = sess.createQuery(queryString);
			requests = queryObject.list();
			if (requests != null && requests.size() > 0) {
	...
	public Request find(Long id) {
		...
		try {
			...
			/*
			 * if(request != null){ Hibernate.initialize(request.getMaps());
			 * Hibernate.initialize(request.getRelations()); }
			 */
			return request;
	...
	public List<Request> findAll(Session sess) {
		...
		requests = query.list();
		/*
		 * if(requests != null && requests.size() > 0){ for (Request s :
		 * requests) { Hibernate.initialize(s.getMaps());
	...
	public String compare(Request current, Request change) throws Exception {
		...
		if (current.getEditedby() != null) {
			...
			result += "Changed by:      " + change.getEditedby() + "["
					+ format.format(change.getEditedtime()) + "]" + "<br>";
		}
		if (!current.getRequestname().equals(change.getRequestname())) {
			result += "Request name:    changed from "
					+ current.getRequestname() + " to "
					+ change.getRequestname() + "<br>";
		}
		...
		if (current.getRequesttime().compareTo(change.getRequesttime()) == -1) {
			DateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm");
			result += "Due date:        changed from "
					+ format.format(current.getRequesttime()) + " to "
					+ format.format(change.getRequesttime()) + "<br>";
		}

		if (current.getIs_public().intValue() != change.getIs_public()
				.intValue()) {
			result += "IsPublic:        changed from \""
					+ publicString(current.getIs_public()) + "\" to \""
					+ publicString(change.getIs_public()) + "\" <br>";
		}

		String signatoryCompare = compareEmails(current.getOwner(),
				change.getOwner(), "signatory");
		...
		String currentForward = StringUtil
				.deFormartCCList(current.getForward());
		...
		String ccCompare = compareEmails(currentForward, changeForward,
				"forward");
		if (ccCompare != null) {
			result += ccCompare;
		}
		...
		if (!current.getDetail().trim().equals(changeDetail_replace.trim())) {
			...
			result += "<br>";
			result += "Description:<br><br>changed from: <br> " + currentDetail
					+ "<br><br>to: <br> " + changeDetail + "<br><br>";
		}
	...
	private String compareProduct(Request current, Request change)
			throws Exception {
		...
		if (currVersion == null && changeVersion != null) {
			return "Added product:   " + changeProduct.getName() + "(version:"
					+ changeVersion.getValue() + ") <br>";
		}
		if (currVersion != null && changeVersion == null) {
			return "Removed product: " + currProduct.getName() + "(version:"
					+ currVersion.getValue() + ") <br>";
		}
		if (currVersion != null
				&& changeVersion != null
				&& currVersion.getId().longValue() != changeVersion.getId()
						.longValue()) {
			return "Product:   changed from " + currProduct.getName()
					+ "(version:" + currVersion.getValue() + ")" + " to "
					+ changeProduct.getName() + "(version:"
					+ changeVersion.getValue() + ") <br>";
		}
		return null;
	}
	...
	private String compareEmails(String current, String change, String type) {
		...
		if (currArray.length != changeArray.length) {
			return changeName + "changed from " + current + " to " + change
					+ " <br>";
		}
	...
	private String compareParent(Request current, Request change)
			throws Exception {
		RequestRelationshipDAO rDao = new RequestRelationshipDAO();
		...
		if (currParentId == null && changeParentId != null) {
			...
			if (changeParent != null) {
				return parent + "added \"" + changeParentId + "  "
						+ changeParent.getRequestname() + "\" <br>";
			}
		}
		...
		if (currParentId != null && changeParentId == null) {
			...
			if (currParent != null) {
				return parent + "removed \"" + currParentId + "  "
						+ currParent.getRequestname() + "\" <br>";
			}
		}
		...
		if (currParentId.longValue() != changeParentId.longValue()) {
			...
			if (currParent != null && changeParent != null) {
				return parent + "changed from \"" + currParentId + "  "
						+ currParent.getRequestname() + "\"   to   \""
						+ changeParentId + "  " + changeParent.getRequestname()
						+ "\" <br>";
			}
		}
	...
	private String compareChild(Request current, Request change)
			throws Exception {
		RequestRelationshipDAO rDao = new RequestRelationshipDAO();
		RequestDAO requestDao = null;
		...
		if ((currChildIdList == null || currChildIdList.length < 1)
				&& StringUtil.isEmpty(change.getChildren())) {
			return null;
		}
		if ((currChildIdList == null || currChildIdList.length < 1)
				&& !StringUtil.isEmpty(change.getChildren())) {
		...
		if (currChildIdList != null && currChildIdList.length > 0) {
			...
			for (String id : currChildIdList) {
				if (!StringUtil.isEmpty(id)) {
					String requestName = requestDao.find(Long.parseLong(id))
							.getRequestname();
		...
		if (currChildIdList.length != childArray.length) {
			return childre + "changed from \"" + currChildList + "\"   to   \""
					+ change.getChildren() + "\" <br>";
		}
	...
	private String compareNotificationType(Request current, Request change) {
		...
		if (maps != null && maps.size() > 0) {
			for (RequestMap m : maps) {
				...
				if (m.getMapname()
						.equals(Request.PROPERTY_REQUEST_NOTIFYCATION)
						&& m.getRequestVersion().intValue() == current
								.getRequestVersion().intValue()) {
					for (RequestMap cm : changeMaps) {
						if (cm.getMapname().equals(
								Request.PROPERTY_REQUEST_NOTIFYCATION)
								&& cm.getRequestVersion().intValue() == change
										.getRequestVersion().intValue()) {
							...
							if (currentEmail.equals(changeEmail)) {
								if (!currentType.equals(changeType)) {
									result += "Signatory:       "
											+ currentEmail
											+ ", changed from \""
											+ getNotificationType(currentType)
											+ "\"  to  \""
											+ getNotificationType(changeType)
											+ "\" <br>";
								}
							}
						}
			...
			for (RequestMap cm : changeMaps) {
				if (cm.getMapname().equals(
					...
					String changeEmail = cm.getMapvalue().split(":")[0];
					String changeType = cm.getMapvalue().split(":")[1];
					result += "Signatory:       " + changeEmail
							+ ", changed to  \""
							+ getNotificationType(changeType) + "\" <br>";
				}
			}
		}
	...
	private String compareFiles(Request current, Request change) {
		...
		if (currMaps != null && currMaps.size() > 0) {
			for (RequestMap m : currMaps) {
				if (m.getRequestVersion().intValue() == current
						.getRequestVersion().intValue()
						&& m.getMapname().equals(
								Request.PROPERTY_REQUEST_ATTACHMENT)) {
					currFileMap
							.put(m.getMapvalue(),
									m.getDynamic().get(
											Request.PROPERTY_ATTACHMENT_MD5));
				}
			}
		}
		if (changeMaps != null && changeMaps.size() > 0) {
			for (RequestMap m : changeMaps) {
				if (m.getRequestVersion().intValue() == change
						.getRequestVersion().intValue()
						&& m.getMapname().equals(
								Request.PROPERTY_REQUEST_ATTACHMENT)) {
					changeFileMap
		...
		/*
		 * if(!currFileMap.isEmpty() && changeFileMap.isEmpty()){
		 * if(currFileMap.size()>1){ result +=
		 * "Following files were deleted: <br> "; }else{ result +=
		 * "Following file was deleted: <br> "; } result +=
		 * convert(currFileMap)+" <br>"; }
		 * 
		 * if(currFileMap.isEmpty() && !changeFileMap.isEmpty()){
		 * if(changeFileMap.size()>1){ result +=
		 * "Following files were added: <br> "; }else{ result +=
		 * "Following file was added: <br> "; } result +=
		 * convert(changeFileMap)+" <br>"; } if(currFileMap.size() !=
		 * changeFileMap.size()){
		 * result+="Attachment was changed from \""+convert
		 * (currFileMap)+"\" to \""+convert(changeFileMap)+"\" <br>"; }
		 */
		...
		for (String currFileName : currFileMap.keySet()) {
			...
			for (String changeFileName : changeFileMap.keySet()) {
				if (currFileName.equals(changeFileName)) {
					removed = false;
					if (!currFileMap.get(currFileName).equals(
							changeFileMap.get(changeFileName))) {
						result += "File: \"" + currFileName + " MD5:"
								+ currFileMap.get(currFileName)
								+ "\" changed to \"" + changeFileName + " MD5:"
								+ changeFileMap.get(changeFileName) + "\"<br>";
					}
				}
			}
			...
			if (removed) {
				result += "File: \"" + currFileName + " MD5:"
						+ currFileMap.get(currFileName) + "\" was deleted <br>";
			}
		}
		...
		for (String changeFileName : changeFileMap.keySet()) {
			...
			if (added) {
				result += "File: \"" + changeFileName + " MD5:"
						+ changeFileMap.get(changeFileName)
						+ "\" was added <br>";
			}
		}
